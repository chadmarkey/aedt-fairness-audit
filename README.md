...
- **A borderline school_tier signal is shared across both LLM
  ...
  correction.
- **The signal does not hold across extractor architectures.** The
  SBERT extractor on the same corpus reads academic_career ×
  school_tier in the *opposite* direction (DI = 1.242, p = 0.18).
  SBERT and the LLM extractor are both within the patent's
  underspecified "users can apply NLP" scope; they disagree on
...
## Known limitations and prospective follow-ons
...
5. **Independent replication.** The audit toolkit is reproducible
   ...
   commands are documented in the Quickstart below.
...
the permutation rep-count reconciliation). Those refinements
narrowed the audit's substantive claim. The follow-ons above are
the next set of moves that would either further narrow it or
strengthen it.
